...
ab -c1000 -n100000 ${FRONTEND_URL}
----

. Apache Bench will take around 30 seconds to complete. Then immediately check the status of Horizontal Pod Autoscaler. To do so, run the following command:
+
[source,sh,role=execute]
----
oc -n microsweeper-ex get horizontalpodautoscaler/microsweeper-appservice-cpu
